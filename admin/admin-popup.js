...
function install(){
  ...
  const preview=()=>{const host=section.querySelector('[data-popup-image-preview]'),url=form.elements.imageUrl.value.trim();host.innerHTML=url?`<img src="${url.replace(/"/g,'&quot;')}" alt="Popup preview">`:'';};
  form.elements.mode.addEventListener('change',syncMode);form.elements.page.addEventListener('change',syncPage);form.elements.imageUrl.addEventListener('input',preview);
  ...
  form.addEventListener('submit',save,true);syncMode();syncPage();void load();
}
async function load(){if(!form||!isAdmin())return;try{const snap=await getDoc(doc(db,'notices',DOC_ID));if(!snap.exists())return;currentDoc=snap.data();const data=unpack(currentDoc.body);if(!data)return;form.elements.active.checked=data.active===true;form.elements.mode.value=data.mode==='image'?'image':'rich';form.elements.titleKo.value=data.titleKo||'';form.elements.titleEn.value=data.titleEn||'';koEditor?.setHTML(data.htmlKo||'');enEditor?.setHTML(data.htmlEn||'');form.elements.imageUrl.value=data.imageUrl||'';form.elements.linkLabelKo.value=data.linkLabelKo||'자세히 보기';form.elements.linkLabelEn.value=data.linkLabelEn||'Learn more';const known=PAGES.some(([v])=>v===data.linkUrl&&v!=='custom');form.elements.page.value=known?data.linkUrl:(data.linkUrl?'custom':'');form.elements.customUrl.value=known?'':(data.linkUrl||'');form.elements.mode.dispatchEvent(new Event('change'));form.elements.page.dispatchEvent(new Event('change'));form.elements.imageUrl.dispatchEvent(new Event('input'));}catch(error){status(`팝업 설정을 불러오지 못했습니다: ${String(error?.message||error)}`,'error');}}
