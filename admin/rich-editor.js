...
export function sanitizeRichHTML(html=''){
  ...
  [...root.querySelectorAll('*')].forEach(el=>{
    ...
    if(el.tagName==='IMG'){
      const src=safeUrl(el.getAttribute('src'));if(!src){el.remove();return;}el.setAttribute('src',src);el.setAttribute('loading','lazy');
    }
    if(el.tagName==='A'){
...
export function createRichEditor(textarea,{uploadImage,placeholder='내용을 입력하세요.'}={}){
  ...
  const sync=()=>{textarea.value=sanitizeRichHTML(canvas.innerHTML);textarea.dispatchEvent(new Event('input',{bubbles:true}));};
  ...
  wrap.querySelector('[data-rich-size]').addEventListener('change',event=>{const value=event.target.value;if(!value)return;canvas.focus();exec('fontSize','7');canvas.querySelectorAll('font[size="7"]').forEach(el=>{el.removeAttribute('size');el.style.fontSize=`${value}px`;});sync();event.target.value='';});
  wrap.querySelector('[data-rich-font]').addEventListener('change',event=>{const value=event.target.value;if(!value)return;canvas.focus();exec('fontName',value);sync();event.target.value='';});
  wrap.querySelector('[data-rich-table]').addEventListener('click',()=>{const rows=Math.max(1,Math.min(20,Number(prompt('표 행 수를 입력하세요.','2'))||0)),cols=Math.max(1,Math.min(10,Number(prompt('표 열 수를 입력하세요.','2'))||0));if(!rows||!cols)return;let html='<table><tbody>';for(let r=0;r<rows;r++){html+='<tr>';for(let c=0;c<cols;c++)html+='<td>&nbsp;</td>';html+='</tr>';}html+='</tbody></table><p><br></p>';insertHTML(canvas,html);sync();});
  wrap.querySelector('[data-rich-image-url]').addEventListener('click',()=>{const url=prompt('이미지 URL을 입력하세요.','https://');const safe=safeUrl(url);if(!safe)return;insertHTML(canvas,`<p><img src="${safe.replace(/"/g,'&quot;')}" alt=""></p>`);sync();});
