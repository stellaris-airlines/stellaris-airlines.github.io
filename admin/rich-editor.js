...
export function createRichEditor(textarea,{uploadImage,placeholder='내용을 입력하세요.'}={}){
  ...
  const wrap=document.createElement('div');wrap.className='admin-rich-editor';wrap.dataset.autoTranslateSkip='true';
  ...
  let dirty=false;
  const sync=(sanitize=true)=>{
    if(sanitize)normalizeFontTags(canvas);
    textarea.value=sanitize?sanitizeRichHTML(canvas.innerHTML):canvas.innerHTML;
    dirty=false;
    return textarea.value;
  };
  const setHTML=html=>{canvas.innerHTML=sanitizeRichHTML(html||'');textarea.value=canvas.innerHTML;dirty=false;};
  const getHTML=()=>sanitizeRichHTML(dirty?canvas.innerHTML:textarea.value||canvas.innerHTML);
  setHTML(textarea.value);
  // Keystrokes only mark the editor dirty. DOMParser sanitization is intentionally deferred
  // until blur/toolbar actions/save so typing cannot lock up the admin page.
  canvas.addEventListener('input',()=>{dirty=true;textarea.value=canvas.innerHTML;},{passive:true});
  canvas.addEventListener('blur',()=>sync(true));
  wrap.querySelectorAll('[data-rich-cmd]').forEach(button=>button.addEventListener('click',()=>{canvas.focus();exec(button.dataset.richCmd);dirty=true;sync(true);}));
  wrap.querySelector('[data-rich-size]').addEventListener('change',event=>{const value=event.target.value;if(!value)return;canvas.focus();exec('fontSize','7');canvas.querySelectorAll('font[size="7"]').forEach(el=>{el.removeAttribute('size');el.style.fontSize=`${value}px`;});dirty=true;sync(true);event.target.value='';});
  wrap.querySelector('[data-rich-font]').addEventListener('change',event=>{const value=event.target.value;if(!value)return;canvas.focus();exec('fontName',value);normalizeFontTags(canvas);dirty=true;sync(true);event.target.value='';});
  wrap.querySelector('[data-rich-table]').addEventListener('click',()=>{const rawRows=prompt('표 행 수를 입력하세요.','2');if(rawRows===null)return;const rawCols=prompt('표 열 수를 입력하세요.','2');if(rawCols===null)return;const rows=Math.max(1,Math.min(20,Number(rawRows)||1)),cols=Math.max(1,Math.min(10,Number(rawCols)||1));let html='<table><tbody>';for(let r=0;r<rows;r++){html+='<tr>';for(let c=0;c<cols;c++)html+='<td>&nbsp;</td>';html+='</tr>';}html+='</tbody></table><p><br></p>';insertHTML(canvas,html);dirty=true;sync(true);});
  wrap.querySelector('[data-rich-image-url]').addEventListener('click',()=>{const url=prompt('이미지 URL을 입력하세요.','https://');const safe=safeImageUrl(url);if(!safe)return;insertHTML(canvas,`<p><img src="${safe.replace(/"/g,'&quot;')}" alt=""></p>`);dirty=true;sync(true);});
  wrap.querySelector('[data-rich-image]').addEventListener('click',()=>file.click());
  file.addEventListener('change',async()=>{const selected=file.files?.[0];file.value='';if(!selected)return;if(!uploadImage){alert('이미지 업로드 기능이 준비되지 않았습니다. 이미지 URL 기능을 사용해 주세요.');return;}const button=wrap.querySelector('[data-rich-image]'),before=button.textContent;button.disabled=true;button.textContent='업로드 중…';try{const url=await uploadImage(selected);insertHTML(canvas,`<p><img src="${String(url).replace(/"/g,'&quot;')}" alt=""></p>`);dirty=true;sync(true);}catch(error){alert(String(error?.message||error));}finally{button.disabled=false;button.textContent=before;}});
  const api={wrap,canvas,getHTML,setHTML,sync:()=>sync(true),clear:()=>setHTML('')};textarea._richEditor=api;return api;
}
