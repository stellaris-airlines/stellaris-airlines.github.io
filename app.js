...
const headerHTML=()=>`<header class="site-header"><div class="header-inner shell-wide">
  ...
  <div class="header-tools">${languageControl()}<a class="book-link" href="${A('book-your-journey/')}">항공권 예약하기</a><a href="${A('find-your-reservations/')}">예약 조회</a><a href="${A('check-in/')}">체크인</a><span class="tool-divider"></span><a data-auth-guest href="${A('login/')}">로그인</a><a data-auth-guest class="signup-link" href="${A('signup/')}">회원가입</a><a data-auth-user href="${A('view-my-starmiles/')}" hidden>내 계정</a><a data-auth-logout href="#" hidden>로그아웃</a><button class="mobile-menu-button" id="mobileMenuButton" type="button" aria-label="메뉴 열기" aria-expanded="false"><span></span><span></span><span></span></button></div>
</div><div class="mobile-nav" id="mobileNav" hidden><a href="${A('book-your-journey/')}">항공권 예약하기</a><a href="${A('find-your-reservations/')}">예약 조회</a><a href="${A('check-in/')}">온라인 체크인</a><a href="${A('boarding-pass/')}">모바일 탑승권</a><a href="${A('my-page/')}">My Page</a><a href="${A('about-us/')}">항공사 정보</a><a href="${A('flight-information/')}">운항 정보</a><a href="${A('destinations/')}">노선 안내</a><a href="${A('travel-info/')}">여행 준비</a><a href="${A('our-fleets/')}">보유 항공기</a><a href="${A('membership/')}">Star Miles</a><a href="${A('view-my-starmiles/')}">내 Star Miles 보기</a><a href="${A('notices/')}">공지사항</a><a href="${A('support/')}">지원 센터</a><a data-auth-guest href="${A('login/')}">로그인</a><a data-auth-guest href="${A('signup/')}">회원가입</a><a data-auth-user href="${A('view-my-starmiles/')}" hidden>내 계정</a><a data-auth-logout href="#" hidden>로그아웃</a></div></header>`;

const footerHTML=()=>`<footer class="site-footer"><div class="shell footer-main"><a class="brand footer-brand" href="${A('')}"><img class="brand-symbol-image" src="${A('assets/stellaris-symbol.png')}" alt=""><img class="brand-wordmark-image" src="${A('assets/stellaris-wordmark.png')}" alt="STELLARIS AIRLINES"></a><div class="footer-columns"><div><strong>항공사</strong><a href="${A('about-us/')}">항공사 소개</a><a href="${A('group/')}">STELLARIS GROUP</a><a href="${A('our-fleets/')}">항공기</a></div><div><strong>여행</strong><a href="${A('book-your-journey/')}">항공권 예약하기</a><a href="${A('find-your-reservations/')}">예약 조회</a><a href="${A('check-in/')}">온라인 체크인</a><a href="${A('boarding-pass/')}">모바일 탑승권</a><a href="${A('destinations/')}">노선 안내</a></div><div><strong>서비스</strong><a href="${A('my-page/')}">My Page</a><a href="${A('travel-info/')}">여행 준비</a><a href="${A('wifi/')}">기내 Wi-Fi</a><a href="${A('inflight-entertainment/')}">기내 엔터테인먼트</a></div><div><strong>지원</strong><a href="${A('notices/')}">공지사항</a><a href="${A('support/')}">지원 센터</a><a href="${A('membership/')}">Star Miles</a><a href="${A('view-my-starmiles/')}">내 Star Miles 보기</a></div></div></div><div class="shell footer-bottom"><span>ⓒ 2026 STELLARIS AIRLINES. All rights reserved.</span><div><a href="${A('terms/')}">이용약관</a><a href="${A('privacy/')}">개인정보처리방침</a></div></div></footer>`;
