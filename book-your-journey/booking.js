...
depart.addEventListener('change',()=>{returnDate.min=depart.value;if(returnDate.value<=depart.value){const d=new Date(depart.value+'T12:00:00');d.setDate(d.getDate()+7);const suggested=dateISO(d);returnDate.value=suggested>returnDate.max?returnDate.max:suggested}});
