...
async function runSync(user,booking){
  if(syncRunning)return;syncRunning=true;retryButton.hidden=true;statusNode.className='sync-status';statusNode.textContent='서버 예약 저장을 확인하고 있습니다…';
  queueMicrotask(()=>window.STELLARIS_AUTO_TRANSLATE?.translate?.());
  try{
    await withTimeout(syncBooking(user,booking));
    statusNode.className='sync-status success';statusNode.textContent='서버 예약 저장이 완료되었습니다. 예약 조회와 온라인 체크인에서 사용할 수 있습니다.';retryButton.hidden=true;
  }catch(error){
    ...
    else if(error?.message==='sync-timeout')statusNode.textContent='예약은 완료되었습니다. 서버 연결이 지연되고 있어 잠시 후 다시 저장할 수 있습니다.';
    else statusNode.textContent='예약은 완료되었습니다. 서버 저장만 완료되지 않아 다시 시도할 수 있습니다.';
